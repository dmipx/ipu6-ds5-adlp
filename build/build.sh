...
PKG_PATH=${PWD_LC}/../artifactory/
PKG=${PWD_LC}/pkg
RESULTS=${PWD_LC}/results/
...
cd ${PWD_LC}/ipu6-drivers
#update uapi header
cp include/uapi/linux/ipu-isys.h /usr/src/linux-headers-5.15.49-for-ubuntu/include/uapi/linux/ipu-isys.h

#build modules
make -j -C /usr/src/linux-headers-5.15.49-for-ubuntu/ M=$(pwd)
chown -R --reference=${PWD_LC} $(pwd)

mkdir -p ${RESULTS}

# copy artifacts
cp ${PWD_LC}/ipu6-drivers/drivers/media/i2c/d4xx.ko ${RESULTS}
cp ${PWD_LC}/ipu6-drivers/drivers/media/i2c/max9295.ko ${RESULTS}
cp ${PWD_LC}/ipu6-drivers/drivers/media/i2c/max9296.ko ${RESULTS}
cp ${PWD_LC}/ipu6-drivers/drivers/media/pci/intel/ipu6/intel-ipu6-isys.ko ${RESULTS}
chown -R --reference=${PWD_LC} ${RESULTS}