...
if [ "$install" = true ]; then
    sudo cp rs-enum.sh /usr/local/bin/rs-enum.sh
    sudo cp d457_bind.sh /usr/local/bin/d457_bind.sh
    sudo cp 99-realsense-d4xx-ipu6.rules /etc/udev/rules.d/
else
    sudo rm /etc/udev/rules.d/99-realsense-d4xx-ipu6.rules
    sudo rm /usr/local/bin/d457_bind.sh
